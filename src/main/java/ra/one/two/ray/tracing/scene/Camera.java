...
public class Camera {
    ...
    public Camera(
        ...
        double viewportHeight = 2.0 * Math.tan(cameraFieldOfViewInRadians / 2) * focusDistance ;
        ...
        lowerLeftCorner = Vec3.subtract(cameraOrigin, Vec3.divide(horizontal, 2)).add(Vec3.divide(vertical, 2).negative()).add(w.scaleUp(focusDistance).negative());
     ...
     */
    Ray getRay(double horizontalScaleOfViewportOffset, double verticalScaleOfViewportOffset) {
        ...
        // Get a random vector in a disc of radius equal to lensRadius in the camera plane
        Vec3 offset = Vec3.multiply(u, rd.getxComponent()).add(Vec3.multiply(v, rd.getyComponent()));
        return new Ray(Vec3.add(cameraOrigin, offset),
                Vec3.add(lowerLeftCorner, Vec3.multiply(horizontal, horizontalScaleOfViewportOffset)).add(Vec3.multiply(vertical, verticalScaleOfViewportOffset)).add(cameraOrigin.negative()).add(offset.negative()));
    }
}
